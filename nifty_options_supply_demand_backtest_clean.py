    ...
    def generate_performance_report(self):
        ...
        if closed_trades:
            ...
            if demand_trades:
                ...
                print(f"   - Demand Zones: {len(demand_trades)} trades, Rs.{demand_pnl:,.2f} P&L, {demand_win_rate:.1f}% win rate")
            
            if supply_trades:
                supply_pnl = sum(t['pnl'] for t in supply_trades)
                supply_win_rate = len([t for t in supply_trades if t['pnl'] > 0]) / len(supply_trades) * 100
                print(f"   - Supply Zones: {len(supply_trades)} trades, Rs.{supply_pnl:,.2f} P&L, {supply_win_rate:.1f}% win rate")

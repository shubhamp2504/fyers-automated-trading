"""
NIFTY OPTIONS SUPPLY & DEMAND STRATEGY DEMO BACKTESTER
Advanced options trading system using institutional supply and demand zones
Standalone version with simulated data for demonstration

Based on Supply & Demand Theory from JeaFX:
...
class NiftyOptionsSupplyDemandDemo:
    """NIFTY Options Supply & Demand Strategy Demo with Simulated Data"""
    ...
    def __init__(self):
        ...
        print("NIFTY OPTIONS SUPPLY & DEMAND STRATEGY DEMO")
        print("=" * 80)
        print("Strategy: Institutional Supply & Demand Zones")
        print("Market: NIFTY Index Options (Simulated Data)")
        print("Starting Capital: Rs.1,00,000")
        print("Risk per Trade: 1%")
        print("=" * 80)
    
    def generate_nifty_data(self, days: int = 30) -> pd.DataFrame:
        """Generate realistic NIFTY price data for backtesting"""
        print(f"\nGENERATING NIFTY SIMULATION DATA")
        print("-" * 50)
        ...
        timestamps = []
        current_date = datetime(2025, 1, 1, 9, 15)  # Start from Jan 1, 2025 at 9:15 AM
        ...
        # Generate price data using realistic parameters
        ...
        for i, timestamp in enumerate(timestamps):
            ...
            trend_factor = 0.0001 * np.sin(i / 100)  # Subtle long-term trend
            volatility = np.random.normal(0, 0.003)  # 0.3% volatility
            
            # Create occasional impulse moves (supply/demand zones)
            if np.random.random() < 0.02:  # 2% chance of impulse move
                impulse_magnitude = np.random.choice([-1, 1]) * np.random.uniform(0.005, 0.015)  # 0.5-1.5% move
                volatility += impulse_magnitude
            
            # Calculate price change
            price_change = current_price * (trend_factor + volatility)
            current_price += price_change
            
            # Calculate OHLC for 5-minute candle
            price_range = current_price * np.random.uniform(0.001, 0.003)  # Candle range
            
            open_price = current_price - np.random.uniform(-price_range/2, price_range/2)
            close_price = current_price + np.random.uniform(-price_range/2, price_range/2)
            ...
            volume = np.random.randint(1000, 5000) * 1000  # Volume in thousands
        ...
        print(f"Generated NIFTY data: {len(df)} candles")
        print(f"Period: {df.index[0]} to {df.index[-1]}")
        print(f"Price range: Rs.{df['low'].min():.2f} - Rs.{df['high'].max():.2f}")
        print(f"Sample data points: {len(df[df['is_indecision']])} indecision candles identified")
    ...
    def identify_supply_demand_zones(self):
        ...
        print(f"Found {len(impulses)} impulse moves")
        ...
        if supply_zones:
            ...
            print(f"Strongest supply zone: Rs.{strongest_supply['low']:.2f}-Rs.{strongest_supply['high']:.2f} ({strongest_supply['strength']:.2f}%)")
        
        if demand_zones:
            strongest_demand = max(demand_zones, key=lambda x: x['strength'])
            print(f"Strongest demand zone: Rs.{strongest_demand['low']:.2f}-Rs.{strongest_demand['high']:.2f} ({strongest_demand['strength']:.2f}%)")
    ...
    def run_backtest_demo(self):
        """Run comprehensive supply & demand options backtesting demo"""
        
        print(f"\nSTARTING SUPPLY & DEMAND OPTIONS BACKTEST DEMO")
        print(f"Starting Capital: Rs.{self.capital:,.2f}")
        print("=" * 80)
        
        # Generate demo data
        self.generate_nifty_data(days=30)
                ...
                for zone in self.demand_zones:
                    if not zone['broken'] and self.check_zone_trigger(current_candle, zone):
                        ...
                        if trade:
                            ...
                            print(f"CALL BUY: {trade['symbol']} @ Rs.{trade['entry_premium']:.2f} "
                                  f"(Spot: Rs.{current_price:.2f}) - Trend: {trend}")
                ...
                for zone in self.supply_zones:
                    if not zone['broken'] and self.check_zone_trigger(current_candle, zone):
                        ...
                        if trade:
                            ...
                            print(f"PUT BUY: {trade['symbol']} @ Rs.{trade['entry_premium']:.2f} "
                                  f"(Spot: Rs.{current_price:.2f}) - Trend: {trend}")
    ...
    def generate_performance_report(self):
        ...
        if closed_trades:
            ...
            if demand_trades:
                ...
                print(f"   - Demand Zone Trades: {len(demand_trades)}, Rs.{demand_pnl:,.2f} P&L, {demand_win_rate:.1f}% win rate")
            
            if supply_trades:
                supply_pnl = sum(t['pnl'] for t in supply_trades)
                supply_win_rate = len([t for t in supply_trades if t['pnl'] > 0]) / len(supply_trades) * 100
                print(f"   - Supply Zone Trades: {len(supply_trades)}, Rs.{supply_pnl:,.2f} P&L, {supply_win_rate:.1f}% win rate")
        ...
        if self.demand_zones:
            print(f"   - Zone Test Rate: {(tested_demand + tested_supply) / (len(self.demand_zones) + len(self.supply_zones)) * 100:.1f}%")
        
        print("=" * 80)

def main():
    """Run NIFTY Options Supply & Demand Strategy Demo"""
    
    # Initialize demo
    demo = NiftyOptionsSupplyDemandDemo()
    
    # Run backtest demo
    demo.run_backtest_demo()
    
    # Generate performance report
    demo.generate_performance_report()
    
    print(f"\nNIFTY OPTIONS SUPPLY & DEMAND STRATEGY DEMO COMPLETE!")
    print("This demonstration shows how the supply & demand strategy")
    print("identifies institutional zones and trades options accordingly.")
